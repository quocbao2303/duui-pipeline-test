 ...
 * 
 * This pipeline processes text through three NLP components:
 * 1. Sentiment - Analyzes emotional tone (positive/negative/neutral)
 * 2. HateCheck - Detects hate speech patterns
 * 3. FactChecking - Verifies claims against facts (requires pre-annotated Claims/Facts)
 */
public class DUUIPipeline {
    ...
    private static void runPipeline() throws Exception {
        ...
        // NOTE: FactChecking requires Claim and Fact annotations in the CAS.
        // We'll add these programmatically before running the pipeline.
    ...
    private static void addClaimsAndFacts(JCas jCas) {
        ...
        // Format: [claim_text, fact_text, claim_start_hint]
        ...
        for (String[] pair : claimsAndFacts) {
            ...
            // Create Fact annotation (using document end as placeholder span)
    ...
    private static void displayResults(JCas jCas) {
        ...
        try {
            // Try TTLab Sentiment type
            Class<?> sentimentClass = Class.forName("org.hucompute.textimager.uima.type.Sentiment");
        ...
        System.out.println("    All annotations: " + count);
        System.out.println("    Hate annotations: " + hate